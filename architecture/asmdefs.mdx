---
title: "Assembly Definitions: Enforcing Layer Boundaries at Compile Time"
...
This page explains the seven assemblies in the project, the rules they encode, and how the Editor tooling enforces them automatically.
...
<Info>
All seven assemblies are **planned** for the pre-production project layout. The folder structure and `.asmdef` files are being established as foundational scaffolding.
</Info>
...
Infrastructure implementations are **bound at runtime** by the VContainer composition root that lives in the Bootstrap scene (inside `SET.Presentation`). The Bootstrap scene is the only place in the entire codebase where a concrete infrastructure type is ever named.
...
Each assembly is represented by a single `.asmdef` JSON file placed at the root of its folder. Here is the file for `SET.Application`:
...
<Info>
The Editor validator script is **planned** (pre-production). The rule it enforces is active design intent, not yet automated.
</Info>

A custom Editor script in `SET.Editor` runs at build time and scans every `.cs` file inside `_Project/Domain/` and `_Project/Application/`. It looks for two banned using directives:
...
The validator is also wired into the CI pipeline and runs on every pull request.
