...
|---------------|--------|
| **Mental model** | Establish the four-layer Clean Architecture model and the single dependency rule before any other reading |
| **Mode differentiation** | Explain how the same codebase serves Single Player, Pass & Play, and Online Multiplayer (**Planned**) by swapping DI bindings, not logic |
| **Non-negotiable rules** | Document the hard boundaries that every layer must respect, backed by compile-time enforcement |
...
These rules come from the Hard Boundaries document and are enforced by `.asmdef` compile-time checks and editor validators:
...
Use this checklist when adding a new cross-layer feature to ensure the architecture is respected:

- [ ] **Domain type added?** Zero external references — no `UnityEngine`, `Nakama`, or `R3` imports
- [ ] **New Application interface defined?** Lives in `Application/Services/` — Infrastructure implements it, Presentation consumes it via DI
- [ ] **New mode or flow added?** State authority model documented — client-owned (local) or server-mirror (online multiplayer, Planned)
- [ ] **No `GameSession` expansion into Infrastructure concerns** — audio, platform, and network belong in their own Infrastructure services
- [ ] **All reactive streams unidirectional** — Application pushes state out; Presentation pushes commands in; no bidirectional callbacks
- [ ] **New DI binding registered in Bootstrap** — no `new ConcreteType()` calls outside the composition root
- [ ] **Unit tests added** for any new Domain or Application logic in `_Tests/EditMode/`
