...
sidebarTitle: "Rules"
description: "Full implementation-ready SET card game rules covering the validation algorithm, board layout, dealing, scoring, end conditions, and every documented edge case."
---
...
At match start, 12 cards are drawn from the top of the shuffled deck and placed face-up in a **4 × 3 grid** (4 columns, 3 rows). Slot indices are assigned left-to-right, top-to-bottom: slot 0 is top-left, slot 11 is bottom-right.
...
After every board mutation (initial deal, refill, or expansion), the system calls `AnySetExists()`. If no valid Set exists **and** the Deck has ≥ 3 cards remaining, a "No Set" notification is displayed briefly and 3 new cards are added:

| Board size before | After expansion |
|---|---|
| 12 cards (4 × 3) | 15 cards (4 × 4, minus one row gap) |
| 15 cards | 18 cards |
| 18 cards | 21 cards (maximum) |
...
2. The Board is at 21 cards, no valid Set exists, and the Deck is empty.
3. *(Timed mode)* The global match timer reaches 0.
4. *(Timed mode + Point penalty)* A time penalty reduces a player's clock to ≤ 0.
